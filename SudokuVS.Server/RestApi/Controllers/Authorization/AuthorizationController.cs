...
public class AuthorizationController : Controller
{
    ...
    public async Task<IActionResult> Authorize()
    {
        ...
        {
            throw new InvalidOperationException("Bad client ID.");
        }
        ...
        AppUser user = await _userManager.GetUserAsync(result.Principal) ?? throw new InvalidOperationException("The user details cannot be retrieved.");
        ...
        string applicationId = await _applicationManager.GetIdAsync(application) ?? throw new InvalidOperationException("Invalid application.");
        ...
        switch (await _applicationManager.GetConsentTypeAsync(application))
        {
            ...
            case OpenIddictConstants.ConsentTypes.Explicit when authorizations.Count is not 0 && !request.HasPrompt(OpenIddictConstants.Prompts.Consent):
                ...
                ClaimsIdentity identity = new(TokenValidationParameters.DefaultAuthenticationType, OpenIddictConstants.Claims.Name, OpenIddictConstants.Claims.Role);
    ...
    public async Task<IActionResult> Accept()
    {
        ...
        // Retrieve the profile of the logged in user.
        AppUser user = await _userManager.GetUserAsync(User) ?? throw new InvalidOperationException("The user details cannot be retrieved.");
    ...
    public async Task<IActionResult> Exchange()
    {
        ...
        string? subjectClaim = result.Principal?.GetClaim(OpenIddictConstants.Claims.Subject);
        AppUser? user = subjectClaim == null ? null : await _userManager.FindByIdAsync(subjectClaim);
        if (user is null)
        {
            return Forbid(
                authenticationSchemes: OpenIddictServerAspNetCoreDefaults.AuthenticationScheme,
